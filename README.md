...
| **Offline banner (broadcaster)** | `POST https://yourdomain.com/api/offline_banner/{roomId}` — JSON `{ "offline_banner": "…" }` (shown on `/watch` when not live; empty clears). Public `GET /api/room_info/{roomId}` includes `offline_banner`. |
